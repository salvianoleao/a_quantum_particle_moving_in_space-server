...
if "a" not in form or "Vo" not in form or "V1" not in form or "d" not in form:
	...
	plt.title('Wavefunctions for even states', fontsize=30)
	...
	for n in range(1,nstates+1):
	    ...
	    if (n%2==0):
	        ...
	        ax.plot(X_lef2, E_vals[n-1]-D*np.exp(a0*X_lef2), color="red", label="", linewidth=2.8)
	        ax.plot(X_lef1, E_vals[n-1]-B*np.cos(k*X_lef1)+C*np.sin(k*X_lef1), color="red", label="", linewidth=2.8)
	        ax.plot(X_mid,  E_vals[n-1]+A*(-np.exp(-a1*X_mid)+np.exp(a1*X_mid)), color="red", label="", linewidth=2.8)
	        ax.plot(X_rig1, E_vals[n-1]+B*np.cos(k*X_rig1)+C*np.sin(k*X_rig1), color="red", label="", linewidth=2.8)
	        ax.plot(X_rig2, E_vals[n-1]+D*np.exp(-a0*X_rig2), color="red", label="", linewidth=2.8)
